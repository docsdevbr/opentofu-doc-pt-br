...
* `login.v1`: [login protocol version 1](../cli/commands/login.mdx)
* `modules.v1`: [module registry API version 1](./module-registry-protocol.mdx)
* `providers.v1`: [provider registry API version 1](./provider-registry-protocol.mdx)

## Authentication

If credentials for the given hostname are available in
[the CLI config](../cli/config/config-file.mdx#Credentials) through a `credentials_helper` or a host-specific environment variable, then they will be included in the request for the discovery document.
