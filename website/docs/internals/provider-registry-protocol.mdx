...
* `hostname` is the registry host that the provider is considered to have
  ...
  [unless overridden in the CLI configuration](../cli/config/config-file.mdx#provider-installation).
...
[the provider installation method configuration](../cli/config/config-file.mdx#provider-installation)
instead.
...
[OpenTofu's remote service discovery protocol](../internals/remote-service-discovery.mdx),
