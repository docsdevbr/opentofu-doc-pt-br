...
[the CLI configuration's `provider_installation` block](../cli/config/config-file.mdx#provider-installation).
...
[the Provider Requirements documentation](../language/providers/requirements.mdx).
...
[the remote service discovery protocol](../internals/remote-service-discovery.mdx) so
...
[credentials](../cli/config/config-file.mdx#credentials) for the hostname
...
[the `tofu providers mirror` subcommand](../cli/commands/providers/mirror.mdx),
