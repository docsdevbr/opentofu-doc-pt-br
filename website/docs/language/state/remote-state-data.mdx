...
:::note
These are some common options from the Official OpenTofu providers, but
...
be used to share data between OpenTofu configurations. See individual provider
...
systems other than OpenTofu, such as configuration management or scheduler
...
* `workspace` - (Optional) The OpenTofu workspace to use, if the backend
  supports workspaces.
