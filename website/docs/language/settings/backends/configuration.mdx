...
When you change a backend's configuration, you must run `tofu init` again
...
The local backend configuration is different and entirely separate from the `terraform.tfstate` file that contains [state data](../../../language/state/index.mdx) about your real-world infrastructure. OpenTofu stores the `terraform.tfstate` file in your remote backend.
...
automatically by an automation script running OpenTofu. When some or all of
...
- **File**: A configuration file may be specified via the `init` command line.
  ...
  `tofu init`. If the file contains secrets it may be kept in
  a secure data store, such as [Vault](https://www.vaultproject.io/),
  in which case it must be downloaded to the local disk before running OpenTofu.
...
- **Interactively**: OpenTofu will interactively ask you for the required
  values, unless interactive input is disabled. OpenTofu will not prompt for
  optional values.
...
present in plain text on local disk when running OpenTofu.

When using partial configuration, OpenTofu requires at a minimum that
an empty backend configuration is specified in one of the root OpenTofu
...
top-level attributes, without the need to wrap it in another `tofu`
or `backend` block:
...
naming pattern. OpenTofu will not prevent you from using other names but following
...
If you're just reconfiguring the same backend, OpenTofu will still ask if you
...
OpenTofu is back to behaving as it does by default.
