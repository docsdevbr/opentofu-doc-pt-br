...
[`try`](../../language/functions/try.mdx) instead, because it allows for more concise definition of
...
[custom variable validation rules](../../language/values/variables.mdx#custom-validation-rules).
For example:
...
prefer to use [`try`](../../language/functions/try.mdx).
:::
...
* [`try`](../../language/functions/try.mdx), which tries evaluating a sequence of expressions and
