...
reusable modules distributed in separate repositories. OpenTofu tooling is
...
* **Variables and outputs should have descriptions.** All variables and
  ...
  [variable configuration](../../../language/values/variables.mdx) and
  [output configuration](../../../language/values/outputs.mdx) for more details.
...
* **Nested modules**. Nested modules should exist under the `modules/`
  ...
  use only. These are purely advisory; OpenTofu will not actively deny usage
  ...
  paths like `./modules/consul-cluster` so that OpenTofu will consider them
  ...
  ideally be [composable](../../../language/modules/develop/composition.mdx) by the caller, rather than
