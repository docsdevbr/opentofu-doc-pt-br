...
OpenTofu makes several kinds of named values available. Each of these names is
...
The main kinds of named values available in OpenTofu are:
...
[attribute notation](types.mdx#indices-and-attributes) for elements of object values, they are not
...
`<RESOURCE TYPE>.<NAME>` represents a [managed resource](../../language/resources/index.mdx) of
...
* If the resource doesn't use `count` or `for_each`, the reference's value is an
  ...
  access them using [dot or square bracket notation](types.mdx#indices-and-attributes).
...
will be interpreted by OpenTofu as a reference to a managed resource.
...
`var.<NAME>` is the value of the [input variable](../../language/values/variables.mdx) of the given name.

If the variable has a type constraint (`type` argument) as part of its
declaration, OpenTofu will automatically convert the caller's given value
...
`local.<NAME>` is the value of the [local value](../../language/values/locals.mdx) of the given name.
...
[a `module` block](../../language/modules/syntax.mdx).
...
[output values](../../language/values/outputs.mdx), use `module.<MODULE NAME>.<OUTPUT NAME>`.
...
[data resource](../../language/data-sources/index.mdx) of the given data
...
- `path.cwd` is the filesystem path of the original working directory from where you
  ran OpenTofu before applying any `-chdir` argument. This path is an absolute path
...
* `count.index`, in resources that use
  [the `count` meta-argument](../../language/meta-arguments/count.mdx).
* `each.key` / `each.value`, in resources that use
  [the `for_each` meta-argument](../../language/meta-arguments/for_each.mdx).
* `self`, in [provisioner](../../language/resources/provisioners/syntax.mdx) and
  [connection](../../language/resources/provisioners/connection.mdx) blocks.
...
If you use [`dynamic` blocks](dynamic-blocks.mdx) to dynamically generate
...
in their block bodies, and OpenTofu analyzes these expressions to automatically
...
* The arguments of the `ebs_block_device` nested blocks can be accessed using
  a [splat expression](splat.mdx). For example, to obtain a list of
...
* Sometimes nested blocks are defined as taking a logical key to identify each
  ...
  block types, use a [`for` expression](for.mdx):
...
either [splat expressions](splat.mdx) or index syntax:
...
be accessed using a [`for` expression](for.mdx).
...
certain attributes as _sensitive_, in which case OpenTofu will show a
...
[an input variable declared as sensitive](../../language/values/variables.mdx#suppressing-values-in-cli-output),
where OpenTofu will hide the value in the plan and apply messages and will
...
[Cases where OpenTofu may disclose a sensitive variable](../../language/values/variables.mdx#cases-where-tofu-may-disclose-a-sensitive-variable).
...
When OpenTofu is planning a set of changes that will apply your configuration,
...
OpenTofu cannot predict the value of this id until the object has been created.

OpenTofu uses special unknown value placeholders for information that it
cannot predict during the plan phase. The OpenTofu language automatically
...
* OpenTofu will attempt to validate that unknown values are of suitable
  types where possible, but incorrect use of such values may not be detected
  until the apply phase, causing the apply to fail.

Unknown values appear in the `tofu plan` output as `(known after apply)`.
