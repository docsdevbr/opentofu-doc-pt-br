...
[Referring to Resource Instances](../../language/meta-arguments/for_each.mdx#referring-to-instances).
...
variables whose default value is `null` to represent the absence of any value. This allows the module to adapt the variable value for OpenTofu language features designed to work with collections. For example:
...
The above example uses a [`dynamic` block](dynamic-blocks.mdx), which
...
Earlier versions of the OpenTofu language had a slightly different version
