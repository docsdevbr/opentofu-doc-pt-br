...
OpenTofu module authors and provider developers can use detailed type
constraints to validate user-provided values for their input variables and
resource arguments. This requires some additional knowledge about OpenTofu's
...
Type constraints look like other kinds of OpenTofu
[expressions](index.mdx), but are a special syntax. Within the
OpenTofu language, they are only valid in the `type` argument of an
[input variable](../../language/values/variables.mdx).

## Primitive Types

A _primitive_ type is a simple type that isn't made from any other types. All
primitive types in OpenTofu are represented by a type keyword. The available
primitive types are:
...
The OpenTofu language will automatically convert `number` and `bool` values
...
The three kinds of collection type in the OpenTofu language are:
...
* `map(...)`: a collection of values where each is identified by a string label.
  ...
  :::note
  Although colons are valid delimiters between keys and values, `tofu fmt` ignores them. In contrast, `tofu fmt` attempts to vertically align equals signs.
  :::
...
The two kinds of structural type in the OpenTofu language are:
...
The OpenTofu language has literal expressions for creating tuple and object
values, which are described in
[Expressions: Literal Expressions](types.mdx#literal-expressions) as
"list/tuple" literals and "map/object" literals, respectively.

OpenTofu does _not_ provide any way to directly represent lists, maps, or sets.
...
user, and most of the OpenTofu documentation conflates lists with tuples and
...
used interchangeably within the OpenTofu language, and most of OpenTofu's
documentation glosses over the differences between the kinds of complex type.
This is due to two conversion behaviors:

* Whenever possible, OpenTofu converts values between similar kinds of complex
...
* Whenever possible, OpenTofu converts _element values_ within a complex type,
...
user provides the tuple `["a", 15, true]`, OpenTofu will internally transform
...
OpenTofu will automatically convert the second and third strings back to the
...
`{name = ["Kristy", "Claudia", "Mary Anne", "Stacey"], age = 12}`, OpenTofu
...
value against a type constraint containing `any`, OpenTofu will attempt to
...
`any` as the placeholder for the element type of a collection then OpenTofu
will attempt to find a single exact element type to use for the resulting
collection.

For example, given the type constraint `list(any)`, OpenTofu will examine
...
`tuple([string, string, string])` -- OpenTofu analyzes this as follows:
...
If the elements of the given tuple are not all of the same type then OpenTofu
will attempt to find a single type that they can all convert to. OpenTofu
will consider various conversion rules as described in earlier sections.

* If the given value were instead `["a", 1, "b"]` then OpenTofu would still
...
* If the given value were instead `["a", [], "b"]` then the value cannot
  ...
  and an empty tuple can convert to. OpenTofu would reject this value,
...
OpenTofu typically returns an error when it does not receive a value for specified object attributes. When you mark an attribute as optional, OpenTofu instead inserts a default value for the missing attribute. This allows the receiving module to describe an appropriate fallback behavior.
...
- **Default:** (Optional) The second argument defines the default value that OpenTofu should use if the attribute is not present. This must be compatible with the attribute type. If not specified, OpenTofu uses a `null` value of the appropriate type as the default.

An optional attribute with a non-`null` default value is guaranteed to never have the value `null` within the receiving module. OpenTofu will substitute the default value both when a caller omits the attribute altogether and when a caller explicitly sets it to `null`, thereby avoiding the need for additional checks to handle a possible null value.

OpenTofu applies object attribute defaults top-down in nested variable types. This means that OpenTofu applies the default value you specify in the `optional` modifier first and then later applies any nested default values to that attribute.
...
The `production` bucket does not specify the index and error documents, and the `archived` bucket omits the website configuration entirely. OpenTofu will use the default values specified in the `bucket` type constraint.
...
- For the `production` and `docs` buckets, OpenTofu sets `enabled` to `true`. OpenTofu also supplies default values for `website`, and then the values specified in `docs` override those defaults.
- For the `archived` and `docs` buckets, OpenTofu sets `routing_rules` to a `null` value. When OpenTofu does not receive optional attributes and there are no specified defaults, OpenTofu populates those attributes with a `null` value.
- For the `archived` bucket, OpenTofu populates the `website` attribute with the default values specified in the `buckets` type constraint.
