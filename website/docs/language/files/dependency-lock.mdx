...
description: >-
  OpenTofu uses the dependency lock file .terraform.lock.hcl to track and select
  provider versions. Learn about dependency installation and lock file changes.
---

# Dependency Lock File

An OpenTofu configuration may refer to two different kinds of external
dependency that come from outside of its own codebase:

- [Providers](../../language/providers/requirements.mdx), which are plugins for OpenTofu
  that extend it with support for interacting with various external systems.
- [Modules](../../language/modules/index.mdx), which allow
  splitting out groups of OpenTofu configuration constructs (written in the
  OpenTofu language) into reusable abstractions.

Both of these dependency types can be published and updated independently from
OpenTofu itself and from the configurations that depend on them. For that
reason, OpenTofu must determine which versions of those dependencies are
potentially compatible with the current configuration and which versions are
currently selected for use.

[Version constraints](../../language/expressions/version-constraints.mdx) within the configuration
itself determine which versions of dependencies are _potentially_ compatible,
but after selecting a specific version of each dependency OpenTofu remembers
...
OpenTofu does not remember version selections for remote modules, and so
OpenTofu will always select the newest available module version that meets
the specified version constraints. You can use an _exact_ version constraint
to ensure that OpenTofu will always select the same module version.
...
OpenTofu automatically creates or updates the dependency lock file each time
you run [the `tofu init` command](../../cli/commands/init.mdx). You should
...
When `tofu init` is working on installing all of the providers needed for
a configuration, OpenTofu considers both the version constraints in the
configuration _and_ the version selections recorded in the lock file.

If a particular provider has no existing recorded selection, OpenTofu will
...
OpenTofu will always re-select that version for installation, even if a
newer version has become available. You can override that behavior by adding
the `-upgrade` option when you run `tofu init`, in which case OpenTofu
will disregard the existing selections and once again select the newest
available version matching the version constraint.

If a particular `tofu init` call makes changes to the lock file, OpenTofu
...
[review the changes OpenTofu has proposed in the file](#understanding-lock-file-changes),
...
OpenTofu will also verify that each package it installs matches at least one
...
regulations, and then trust that OpenTofu will raise an error if a future
run of `tofu init` encounters a non-matching package for the same
provider version.
...
- If you install a provider from an origin registry which provides checksums
  that are signed with a cryptographic signature, OpenTofu will treat all
...
- If you install a provider for the first time using an alternative
  installation method, such as a filesystem or network mirror, OpenTofu will
  not be able to verify the checksums for any platform other than the one
  where you ran `tofu init`, and so it will not record the checksums
  ...
  [the `tofu providers lock` command](../../cli/commands/providers/lock.mdx),
  which will then allow future calls to `tofu init` to verify that the
...
OpenTofu itself, rather than being updated manually by you or your team,
your version control system may show you that the file has changed.

There are a few different types of changes that OpenTofu can potentially make
...
[provider requirements](../../language/providers/requirements.mdx) for any module in your
configuration, or if you add an external module that includes a new provider
dependency itself, `tofu init` will respond to that by selecting the
...
- `version`: the exact version that OpenTofu selected based on the version
  constraints in the configuration.
- `constraints`: all of the version constraints that OpenTofu considered when
  making this selection. (OpenTofu doesn't actually use this information to
...
If you run `tofu init -upgrade` to ask OpenTofu to consider newer provider
versions that still match the configured version constraints, OpenTofu may
...
a change to the configured version constraints, OpenTofu will also record
...
The addition of a new checksum into the `hashes` value represents OpenTofu
...
- `zh:`: a mnemonic for "zip hash", this is a legacy hash format which is
  part of the OpenTofu provider registry protocol and is therefore used for
  ...
  [provider installation methods](../../cli/config/config-file.mdx#provider-installation),
...
- `h1:`: a mnemonic for "hash scheme 1", which is the current preferred hashing
  ...
  Due to the limited scope of the `zh:` scheme, OpenTofu will
...
OpenTofu will add a new hash to an existing provider only if the hash is
calculated from a package that _also_ matches one of the existing hashes. In
the above example, OpenTofu installed a `hashicorp/azurerm` package for a
...
After confirming the `zh:` checksum match, OpenTofu then recorded the
...
existing `provider` block for it), OpenTofu will pre-populate the `hashes`
...
will consist primarily of hashes using that scheme, which OpenTofu will then
...
can ask OpenTofu to pre-populate hashes for a chosen set of platforms
using
[the `tofu providers lock` command](../../cli/commands/providers/lock.mdx):
...
the case where OpenTofu will learn about a `h1:` equivalent only at a later
time. See the `tofu providers lock` documentation for more information on
this command.

### Providers that are no longer required

To determine whether there still exists a dependency on a given provider,
OpenTofu uses two sources of truth: the configuration itself, and the state.
If you remove the last dependency on a particular provider from both your
configuration and state, then `tofu init` will remove any existing lock
...
`tofu init` again, OpenTofu will treat it as if it were
