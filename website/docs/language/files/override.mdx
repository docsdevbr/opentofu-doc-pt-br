...
human-edited configuration file in the OpenTofu language native syntax
...
each top-level block defined in an override file, OpenTofu attempts to find
...
If you have an OpenTofu configuration `example.tf` with the following contents:
...
OpenTofu will merge the latter into the former, behaving as if the original
...
the variable's `type`, OpenTofu attempts to convert the default value to
...
set in the override file, OpenTofu will use the `backend` block specified in the override file upon merging.
Similarly, if a `backend` block is set within the original configuration and a `cloud` block
is set in the override file, OpenTofu will use the `cloud` block specified in the override
file upon merging.
